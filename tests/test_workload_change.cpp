...
int search_disk_index(int argc, char** argv, diskann::Parameters& paras,
  ...
  for (int i = 0; i < rounds; ++i) {
    ...
    LOG(INFO) << "Range [" << r_start << ", " << r_end << ")";
    auto truthset_bin =
        truthset_bin_prefix + "/gt_" + std::to_string(r_start) + ".bin";
    if (gt_ids) {
      delete[] gt_ids;
      delete[] gt_dists;
      gt_ids = nullptr;
...
#pragma omp parallel for schedule(dynamic, 1)
      ...
      LOG(INFO) << "Ground truth file " << truthset_bin
                << " not found. Skipping recall calculation";
    }

    auto st = std::chrono::high_resolution_clock::now();
#pragma omp parallel for num_threads(num_threads)
    for (int j = r_start; j < r_start + vector_per_round; ++j) {
      uint32_t vtag = j;
      sync_index.lazy_delete(vtag);
      // sync_index.delete_in_place(vtag);
    }
    auto mi = std::chrono::high_resolution_clock::now();
#pragma omp parallel for num_threads(num_threads)
    ...
    LOG(INFO) << "Throughput of deletion: "
              << 1000 * (double) vector_per_round /
                     std::chrono::duration_cast<std::chrono::milliseconds>(mi -
                                                                           st)
                         .count()
              << " vec/s, insertion: "
              << 1000 * (double) vector_per_round /
