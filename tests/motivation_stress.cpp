...
#else
#define InsertQPS 12000000
#endif

#define STRESS_INSERT_THREADS 12
#define STRESS_VECTOR_OFFSET 700000000
#define STRESS_VECTOR_R_OFFSET 800000000
#define BATCH_SIZE 1000000
bool in_stress_test = true;
...
#ifdef DUMMY_STRESS
  LOG(INFO) << "Get trace R: " << r_start << " insert size "
            << insert_tags.size() << " Delete tags 0: " << delete_tags[0]
            << " insert tags 0: " << insert_tags[0];
#endif
...
void update(const std::string& data_bin, const unsigned L_disk,
  ...
  int               batch = 200;  // step / 10;
  int               inMemorySize = 0;
  std::future<void> merge_future;

  fprintf(stderr, "Need copy: %d, need swap: %d\n", ckpt == 0, ckpt % 2 != 0);
#ifndef DUMMY_STRESS
  diskann::MergeInsert<T, TagT>* sync_index = new diskann::MergeInsert<T, TagT>(
      paras, dim, save_path + "_mem", save_path, save_path + "_merge", dist_cmp,
      metric, false, save_path, ckpt == 0, ckpt % 2 != 0, 0);
#else
  diskann::MergeInsert<T, TagT>* sync_index = nullptr;
#endif

  uint64_t vecs_per_step = 1000000;

  uint64_t ckpt_i = 0, res = 0;
...
#endif
  LOG(INFO) << "vecs per step: " << vecs_per_step << " ckpt_i: " << ckpt_i
            << " res: " << res;
  ...
  for (int i = ckpt_i; i < batch; i++) {
    ...
    get_trace<T, TagT>(data_bin, st, st + STRESS_VECTOR_R_OFFSET, vecs_per_step,
                       delete_vec, insert_vec, data_load);

    std::future<void> insert_future = std::async(
        std::launch::async, insertion_kernel<T, TagT>, data_load.data(),
        sync_index, std::ref(insert_vec), aligned_dim);
